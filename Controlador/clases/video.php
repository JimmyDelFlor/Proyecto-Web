...
class Video {
  ...
  private function getSpanishDate($date, $includeTime = false) {
    $timestamp = strtotime($date);
    $format = $includeTime ? 'j \\d\\e F \\d\\e Y, H:i:s' : 'j \\d\\e F \\d\\e Y';
  ...
  public function getUploadDate() {
    return $this->getSpanishDate($this->sqlData["uploadDate"]);
  }

  public function getTimeStamp() {
    return $this->getSpanishDate($this->sqlData["uploadDate"], true);
  }
  ...
  public function getComments() {
    ...
    while($row = $query->fetch(PDO::FETCH_ASSOC)) {
      $comment = new comentario($this->con, $row, $this->userLoggedInObj, $id);
